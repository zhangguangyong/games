import {ActionType, Direction, IAction, IGame, ISetting, SquareType, Status} from 'pages/Snake/types'
...
export const gameReducer = (state: IGame, action: IAction) => {
  ...
  // 改变头部
  map.set(toPositionKey(newHeader), SquareType.HEADER)
  // 是否吃到了食物
  let eatenFood = toPositionKey(newHeader) === toPositionKey(food)
  // 设置原来的头部
  map.set(toPositionKey(header), eatenFood ? SquareType.BODY : SquareType.DEFAULT)
  // 身体
  let newBody = [...body]
  // 吃到食物
  if (eatenFood) {
    ...
    newBody = [header, ...newBody]
    map.set(toPositionKey(header), SquareType.BODY)
    map.set(toPositionKey(removed), SquareType.DEFAULT)
  }
  ...
  if (eatenFood) {
    ...
    map.set(toPositionKey(newFood), SquareType.FOOD)
  }
...
export const reset = (setting: ISetting): IGame => {
  ...
  for (let i = 0; i < setting.rows; i++) {
    for (let j = 0; j < setting.cols; j++) {
      let key = i + '-' + j
      map.set(key, SquareType.DEFAULT)
    }
  }
  ...
  for (const key of keys) {
    if (key === headerKey) {
      map.set(key, SquareType.HEADER)
      continue
    }
    if (key === foodKey) {
      map.set(key, SquareType.FOOD)
      continue
    }
    map.set(key, SquareType.DEFAULT)
  }
