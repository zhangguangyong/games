import {ActionType, Direction, IAction, IGame, ISetting, Status} from './types'
...
const draw = (game: IGame) => {
  const {map} = game
  let {position, props} = generate(game)
  map.set(toPositionKey(position), props.num)
}
...
export const generate = (game: IGame): ICell => {
  ...
  let randomKey = _.sample(keys.filter(item => map.get(item) === Const.EMPTY))
  let randomNum = _.sample([2, 4, 2, 8, 2, 4, 2])!
  return {position: fromPositionKey(randomKey!), props: {num: randomNum}}
}
