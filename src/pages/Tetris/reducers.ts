...
export const gameReducer = (state: IGame, action: IAction) => {
  ...
  if (type === ActionType.DRAW) {
    ...
    state.next!.points?.forEach(v => map.set(toPositionKey(v), state.next.type))
...
const redraw = (oldShape: IShape, newShape: IShape, map: Map<string, string>) => {
  oldShape?.points?.forEach(v => map.set(toPositionKey(v), Const.EMPTY))
  newShape.points?.forEach(v => map.set(toPositionKey(v), newShape.type))
}
...
const check = (oldShape: IShape, newShape: IShape, game: IGame) => {
  // 不能触碰到已存在的点
  let oldShapePoints = oldShape.points?.map(v => toPositionKey(v))!
  let newShapePoints = newShape.points?.map(v => toPositionKey(v))!
  let map = game.map
